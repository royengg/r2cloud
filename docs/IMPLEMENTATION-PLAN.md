...
The newer [delivery plan](DELIVERY-PLAN.md) supersedes this snapshot. Team invitations/project access administration and repository App authorization/verified attachment are now implemented. Remaining: setup-command profiles and ongoing repository revocation handling in phase 3, the live Codex supervisor in phase 4, and phases 5–7. See [Vercel integration boundaries](VERCEL-SANDBOX.md). These are remaining implementation tasks, not merely missing credentials. OAuth credentials also remain necessary to sign into the running product.
